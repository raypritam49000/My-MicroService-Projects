...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@RequestBody UserDTO userDTO) {
        ...
        if (userExistByUsername) {
            return new ResponseEntity<>(new ApiResponse(false,"Conflict",409,"User already exists with username : " + userDTO.getUsername()), HttpStatus.OK);
        }

        boolean userExistByEmail = userService.userExistsByEmail(userDTO.getEmail());

        if (userExistByEmail) {
            return new ResponseEntity<>(new ApiResponse(false,"Conflict",409,"User already exists with email : " + userDTO.getEmail()), HttpStatus.OK);
        }
